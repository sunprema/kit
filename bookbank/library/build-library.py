...
self-contained HTML), then GENERATES the front door from every book.json:
  ...
  <out>/assets/library.js
  <out>/assets/webmcp-book.js   WebMCP tools every book page loads (see below)
...
Every published book page is also stamped with a small fixed "⌂ Library"
corner chip linking back to the shelf (books themselves carry no such link —
they must stay self-contained for file://, where no shelf exists). The chip
hides itself on file:// and binds Esc as a shortcut back to the shelf.

The shelf (LIBRARY_JS) and every book page (BOOK_WEBMCP_JS, carried by the
chip) also register WebMCP tools on document.modelContext, so browser agents
get search/open/read/navigate tools instead of scraping the DOM.
...
def cover_rel(book_dir: Path, book_id: str):
    ...
    for stem in ("cover", "assets/img/cover-art", "assets/img/cover"):
        for ext in (".webp", ".png"):
            if (book_dir / (stem + ext)).is_file():
                return f"books/{book_id}/{stem}{ext}"
    return None
...
def webp_size(path: Path):
    """(width, height) of a WebP (VP8/VP8L/VP8X) from its header, no deps."""
    try:
...
def image_size(path: Path):
    """(width, height) of a PNG/JPEG/WebP by extension, (None, None) otherwise."""
...
def inject_head(html_text, block):
    """Insert `block` just after <meta charset> — charset must stay ahead of
    any non-ASCII byte (og:description often carries em-dashes; the file://
    WKWebView mojibake gotcha) — else just after the opening <head> tag.
    Replaces any prior generated block so re-publishing is byte-stable (no
    spurious git churn)."""
    import re
    ...
    mo = (re.search(r"<meta\s+charset[^>]*>", html_text, flags=re.I)
          or re.search(r"<head\b[^>]*>", html_text, flags=re.I))
    if mo:
        i = mo.end()
        return html_text[:i] + "\n" + block + html_text[i:]
...
# ---- "Back to the Library" + Share corner chips ----------------------------
# Books deliberately have no link above their own cover/TOC (they must stay
# self-contained and work from file://, where no shelf exists). On the
# published site there IS a shelf one level up, so stamp every book page with
# a small fixed bar at publish time — all books get it retroactively, and
# local file:// copies are untouched (the bar hides itself when it detects
# file://, where the shelf target may not exist and there's no shareable URL).
# The bar carries two pills: a "Library" link back to the shelf, and a "Share"
# button that opens the OS share sheet (navigator.share, i.e. the native PWA
# share on phones/installed apps) — falling back to copy-link where the Web
# Share API is missing, and hiding itself where neither works.

CHIP_BEGIN = "<!-- shelf-chip:begin (generated by build-library) -->"
CHIP_END = "<!-- shelf-chip:end -->"


BOOK_WEBMCP_JS = r"""// WebMCP: register this book page as tools on document.modelContext
...
def chip_block(shelf_href, book_root="./", assets_href="../../assets/"):
    """The corner-bar markup+style+behavior, self-contained so it can't collide
    with or depend on any book's own CSS. Deterministic — republishing is
    byte-stable. Bottom-left: the top edge is where the ash-devices-model
    sticky topbar puts the book title, and bottom-center is the books' own
    prev/next nav. The Share pill starts `hidden` and is only revealed by the
    script once it confirms a share/copy path exists, so no-JS or unsupported
    browsers never see a dead button.

    Also loads the page's WebMCP registration — the shared
    <site>/assets/webmcp-book.js (BOOK_WEBMCP_JS) — so every book page
    registers read/search/navigate tools on document.modelContext. One shared
    file rather than an inline copy: ~11 KB × every page would bloat the repo
    and the offline caches. `book_root` is the page-relative href of the
    book's own directory ("./" for root pages, "../" for concepts/), used to
    reach book.json and the sibling pages; `assets_href` is the page-relative
    href of the shelf's assets/ dir."""
    webmcp = (f'<script src="{assets_href}webmcp-book.js?v={asset_v()}" '
              f'data-book-root="{book_root}" data-shelf="{shelf_href}"></script>')
    return f"""{CHIP_BEGIN}
<div class="bb-shelf-bar">
<a class="bb-chip bb-shelf-chip" href="{shelf_href}" title="Back to the Library (Esc)" aria-label="Back to the Library">&#8962; Library</a>
...
def inject_chip(html_text, block):
    """Insert `block` just before </body>, replacing any prior generated chip
    so re-publishing is byte-stable (same idempotency idea as inject_head)."""
    import re
    # Strip exactly what a prior run injected (block + its trailing newline) —
    # consuming any surrounding newline the document already had would shift
    # every page by a byte on each republish.
    html_text = re.sub(
        re.escape(CHIP_BEGIN) + r".*?" + re.escape(CHIP_END) + r"\n?",
        "", html_text, flags=re.S)
    mo = re.search(r"</body\s*>", html_text, flags=re.I)
    if mo:
        i = mo.start()
...
def inject_book_chip(out: Path, book_id: str):
    """Stamp the library chip into every page of one published book (its root
    *.html and concepts/*.html — the same page set validate_book.py checks).
    Returns how many pages were stamped."""
    book_dir = out / "books" / book_id
    if not book_dir.is_dir():
        return 0
    pages = sorted(book_dir.glob("*.html"))
    if (book_dir / "concepts").is_dir():
        pages += sorted((book_dir / "concepts").glob("*.html"))
    n = 0
    for p in pages:
        href = Path(os.path.relpath(out / "index.html", p.parent)).as_posix()
        book_root = Path(os.path.relpath(book_dir, p.parent)).as_posix() + "/"
        assets = Path(os.path.relpath(out / "assets", p.parent)).as_posix() + "/"
        p.write_text(inject_chip(p.read_text(encoding="utf-8"),
                                 chip_block(href, book_root, assets)),
                     encoding="utf-8")
        n += 1
    return n
...
def build_catalog(root: Path, out: Path, only):
    ...
    for d in sorted(out_books.iterdir()) if out_books.is_dir() else []:
        ...
        entries.append({
            ...
            "url": f"books/{bid}/",
            # Chapter list + cheatsheet: lets catalog.json stand alone as a
            # machine index (the shelf's WebMCP tools search chapter titles
            # and deep-link into books without fetching each book.json).
            "chapters": [
                {"id": c.get("id") or "", "title": c.get("title") or "",
                 "url": f"books/{bid}/{c['file']}"}
                for c in b.get("concepts", [])
                if c.get("status") == "ready" and c.get("file")],
        })
        if (d / "cheatsheet.html").is_file():
            entries[-1]["cheatsheet"] = f"books/{bid}/cheatsheet.html"
...
def asset_v():
    ...
    return hashlib.sha1((LIBRARY_CSS + LIBRARY_JS + BOOK_WEBMCP_JS)
...
LIBRARY_JS = r"""// Client-side search + voice filter for the shelf. No dependencies.
...
var SHELL = ['./', 'assets/library.css?v=@ASSETV@', 'assets/library.js?v=@ASSETV@',
             'assets/webmcp-book.js?v=@ASSETV@',
...
chapters, cheatsheets). See `WEBMCP.md` for the tool list.
"""
...
def main():
    ...
    # "Back to the Library" corner chip on every book page — must also run
    # before the offline manifests so they capture the final page bytes.
    chipped = 0
    for e in entries:
        chipped += inject_book_chip(out, e["id"])
    print(f"Stamped the library chip on {chipped} book page(s).")
    ...
    (out / "assets" / "webmcp-book.js").write_text(BOOK_WEBMCP_JS, encoding="utf-8")
