...
# ---- "Back to the Library" corner chip -------------------------------------
...
# a small fixed chip back to it at publish time — all books get it
# retroactively, and local file:// copies are untouched (the chip hides
# itself when it detects file://, where its target may not exist).
...
def chip_block(shelf_href):
    """The chip markup+style+behavior, self-contained so it can't collide with
    or depend on any book's own CSS. Deterministic — republishing is
    ...
    prev/next nav."""
    return f"""{CHIP_BEGIN}
<a class="bb-shelf-chip" href="{shelf_href}" title="Back to the Library (Esc)" aria-label="Back to the Library">&#8962; Library</a>
<style>
